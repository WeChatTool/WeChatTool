...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="wechattool-accessibility-") as temporary:
        ...
        check("recall-only leaves accessibility unchanged", plan(["recall"]), (0, 0, 0))
        legacy = plan(["recall"])
        del legacy["features"]
        del legacy["hooks"][0]["feature"]
        check("legacy default stays recall-only", legacy, (0, 0, 0))
        check("accessibility-only preserves recall behavior and null guard", ax, (1, 7, 1))
        check("both features work together", both, (0, 7, 1))
        reverse = copy.deepcopy(both)
        reverse["hooks"].reverse()
        check("accessibility pair preflight supports reversed hook order", reverse, (0, 7, 1))
