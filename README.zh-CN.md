...
让撤回的微信消息，留在你的 Mac 上。WeChatTool 通过图形安装器提供防撤回功能和可选的实验性辅助功能支持。每个应用副本都可以单独选择其中一项或同时安装两项。
...
- **防撤回**：默认选中，让撤回的消息在本机保持可见。
- **辅助功能支持（实验性）**：默认关闭，让本地界面自动化工具可以使用辅助功能接口和操作。此功能不会阻止账号安全退出登录，也不能代替 macOS 辅助功能授权。

更改选项后，安装器会自动重新检查兼容性。也可以点击 **检查兼容性**。此操作只读取应用，不会修改微信或聊天数据。如果所选功能不受支持，请更改功能选项或继续使用原版应用。
...
如果选择了防撤回，请让另一个账号发送一条新的文字消息，然后撤回。检查原消息是否仍然可见，并重新打开会话确认。如果选择了辅助功能支持，请在启动后测试辅助功能工具；实际登录会话中的效果尚未验证。也请确认能正常收发消息，并在重新打开此副本后查看其中的新聊天记录。
...
微信 **4.1.15（构建版本 270099）** 的防撤回功能已通过 Apple Silicon 和 Intel 架构的兼容性检查。原生代码测试已在 Apple Silicon 上运行，Intel 架构的测试则通过 Rosetta 运行。这不代表所有微信功能、消息类型或未来版本都能正常使用。

实验性辅助功能支持仅适用于已验证的 **4.1.15 / 270099** 二进制配置，包含 Apple Silicon 和 Intel 架构。安装器会为应用内的每种架构检查所选功能，并拒绝不支持的组合。静态兼容性检查不能证明自动化在实际会话中可用；此选项不会禁用服务器端账号限制，也不会阻止“为了账号安全，请重新登录”提示。
...
| 消息仍被撤回 | 确认安装时选择了防撤回功能，且打开的是生成的副本。可按下方可选步骤查看运行日志。 |
| 已选择辅助功能支持，但自动化仍失败 | 检查自动化工具的 macOS 辅助功能权限。此功能为实验性功能，不能恢复已过期或被拒绝的登录会话。 |
...
命令行默认仅启用防撤回。分析和创建副本时，请使用相同的 `--features` 选项；可以选择 `recall`、`accessibility`，或同时选择两项：
...
如果只需要辅助功能支持，请在两个命令中使用 `--features accessibility`。更改已有副本的功能需要创建新副本。
...
- 防撤回功能只影响这台 Mac 上的显示，不会改变其他会话参与者或设备看到的内容。
