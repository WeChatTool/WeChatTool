...
支持多个微信账号同时使用，**每个安装副本登录一个账号**。各副本的聊天记录、登录状态和设置独立保存，原版应用保持不变。
...
点击 **创建微信副本**，为此账号的应用选择名称和保存位置。默认位置是个人文件夹中的 `Applications/WeChatTool-WeChat.app`。其他微信应用可以保持运行。
...
再次选择同一个未经修改的官方应用，点击 **创建微信副本**，并使用不同名称，例如 **WeChat-Work.app** 或 **WeChat-Personal.app**。在每个安装副本中登录不同账号，即可与其他副本及官方应用同时运行。各自的聊天数据、设置和登录状态独立保存。
